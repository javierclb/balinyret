...
def main(spark,anio,mes,path,sep="/"):
	...
	BASEPRE = spark.read.options(header=True,ignoreLeadingWhiteSpace=True,inferSchema=True,)\
		    .csv(hdfs_path+'/BASE.csv')

    	BASELTPRE = spark.read.options(header=True,ignoreLeadingWhiteSpace=True,inferSchema=True,)\
		     .csv(hdfs_path+'/BASE_LT.csv')
    
    	ZONAS = spark.read.options(header=True,ignoreLeadingWhiteSpace=True,inferSchema=True,)\
		 .csv(hdfs_path+'/Zonas.csv')

    	PRORRATAS = spark.read.options(header=True,ignoreLeadingWhiteSpace=True,inferSchema=True,)\
		      .csv(hdfs_path+'/Prorratas.csv')
    	FECHAS = spark.read.options(header=True,ignoreLeadingWhiteSpace=True,inferSchema=True,)\
		  .csv(hdfs_path+'/Fechas.csv')
    
    	CLIENTESLDX = spark.read.options(header=True,ignoreLeadingWhiteSpace=True,inferSchema=True,)\
		       .csv(hdfs_path+'/Baseclientesldx.csv')
    
    	CMGPRE = spark.read.options(header=True,ignoreLeadingWhiteSpace=True,inferSchema=True,)\
		  .csv(hdfs_path+'/CMG.csv')
    	MEDIDASPRE = spark.read.options(header=True,ignoreLeadingWhiteSpace=True,inferSchema=True,)\
		      .csv(hdfs_path+'/MEDIDAS.csv')
    
    	CMGDOLPRE = spark.read.options(header=True,ignoreLeadingWhiteSpace=True,inferSchema=True,)\
			.csv(hdfs_path+'/cmgdol.csv')
    ...
    #Seleccionamos los campos necesarios de la tabla de costos marginales.
    	...
    	CMGPRE = CMGPRE.withColumnRenamed("nombre_barra","barra").withColumnRenamed("CMG_PESO_KWH","cmg_pesos")\
                   .withColumnRenamed("Hora_Mensual","hora")
    ...
    #Realizamos el join correspondiente, teniendo el costo marginal para cada barra de manera horaria.
    	BASE = BASE.join(CMGPRE,(BASE.barra==CMGPRE.barra)&(BASE.hora==CMGPRE.hora),"inner")\
		.drop(CMGPRE.barra).drop(CMGPRE.hora)
    ...
    #Realizamos manejo de datos en BASE_LT, de manera que sacamos lo que necesitamos.
    	...
    	BASELTPRE = BASELTPRE.withColumnRenamed("clave_LT", "linea")\
			 .withColumnRenamed("nombre_barra_ini","barra_ini")\
			 .withColumnRenamed("nombre_barra_fin","barra_fin")
    ...
    #STRING RELACIONADO A AGREGAR UNA TABLA EN HIVE
    	str_mes2 = 'balinyret.balance'+anio+mes
